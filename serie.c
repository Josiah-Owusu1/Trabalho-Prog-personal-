...
void printaSerie(void *dado){
    ...
    // Para séries, a duração a ser exibida é a total (duração do ep * num de eps)
    ...
    // Formato de impressão é genérico para todos os conteúdos na busca
    printf("SERIE %s; %s; %d:%02d; %s; %d; %s\n",
        serie->id, serie->titulo, horas, minutos, descIdioma, serie->anoLancamento, serie->produtora);
}
